...
./farm -n 2 -q 1 file* -d testdir -t 100 | grep "file*" | awk '{print $1,$2}' | diff - expected.txt
if [[ $? != 0 ]]; then
    echo "test1 failed"
else
    echo "test1 passed"
...
./farm -d testdir -n 8 -q 16 -t 100 file* | grep "file*" | awk '{print $1,$2}' | diff - expected.txt
if [[ $? != 0 ]]; then
    echo "test2 failed"
else
    echo "test2 passed"
